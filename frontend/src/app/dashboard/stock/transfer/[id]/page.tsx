...
import { fetchTransfer, fetchCompany } from '@/lib/api';
import { Button } from '@/components/ui/button';
import { Loader2, ArrowLeft } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import dynamic from 'next/dynamic';
import TransferDocument from '@/components/stock/TransferDocument';
...
export default function TransferViewPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const [transferData, companyData] = await Promise.all([
                    fetchTransfer(params.id as string),
                    fetchCompany()
                ]);
            ...
            <div className="flex-1 p-4 md:p-8 overflow-hidden">
                <div className="h-full w-full max-w-5xl mx-auto shadow-2xl rounded-lg overflow-hidden bg-gray-500">
                    <PDFViewer width="100%" height="100%" className="border-none" showToolbar={true}>
                        <TransferDocument transfer={transfer} company={company} logoUrl={logoUrl} />
                    </PDFViewer>
                </div>
            </div>
        </div>
